/**
 * Pay Base Mainnet canary using the Arc testnet buyer key as payer when seller != arc buyer.
 * Requires the payer wallet to hold Base Mainnet USDC + ETH.
 */
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { dirname, resolve } from "node:path";
import { x402Client } from "@x402/core/client";
import { x402HTTPClient } from "@x402/core/client";
import { registerExactEvmScheme } from "@x402/evm/exact/client";
import { toClientEvmSigner } from "@x402/evm";
import { createPublicClient, http } from "viem";
import { base } from "viem/chains";
import { privateKeyToAccount } from "viem/accounts";

const TARGET_URL =
  process.env.BASE_MAINNET_TARGET_URL ??
  "https://ledgerguard-gules.vercel.app/v1/paid/base/evidence";
const EVIDENCE_FILE = resolve("tmp/base-mainnet-canary-evidence.json");
const CANARY_EVIDENCE_BODY = {
  network: "arcTestnet",
  txHash:
    process.env.BASE_MAINNET_CANARY_TX ??
    "0x2b536c8c0c6789482c0792290c1f310cb5a75532247ac394270707015c02098b",
  intent: {
    action: "transfer",
    expectedDebitAddress: "0x257713534b81f053200c94ecEFDc0aAfa92dF68F",
    expectedRecipient: "0xF1437d9CD304aE49f2ec005AC967813B3a7c466c",
    expectedAssetAddress: "0x3600000000000000000000000000000000000000",
    expectedAmountMicroUsdc: "10000",
    purpose: "Base Mainnet canary",
  },
};

const privateKey = readFileSync(resolve(".env.x402-buyer.local"), "utf8")
  .trim()
  .split("=", 2)[1];
const account = privateKeyToAccount(privateKey);
const publicClient = createPublicClient({
  chain: base,
  transport: http("https://mainnet.base.org"),
});
const client = new x402Client();
registerExactEvmScheme(client, {
  signer: toClientEvmSigner(account, publicClient),
  schemeOptions: { 8453: { rpcUrl: "https://mainnet.base.org" } },
  networks: ["eip155:8453"],
});
const httpClient = new x402HTTPClient(client);
const bodyText = JSON.stringify(CANARY_EVIDENCE_BODY);

const challenge = await fetch(TARGET_URL, {
  method: "POST",
  headers: { "content-type": "application/json" },
  body: bodyText,
});
if (challenge.status !== 402) {
  throw new Error(`Expected HTTP 402, got ${challenge.status}: ${await challenge.text()}`);
}

const paymentRequired = httpClient.getPaymentRequiredResponse((name) =>
  challenge.headers.get(name),
);
const payTo = paymentRequired.accepts?.[0]?.payTo;
if (payTo?.toLowerCase() === account.address.toLowerCase()) {
  throw new Error(`Self-payment blocked for ${account.address}`);
}

console.log(JSON.stringify({ payer: account.address, payTo }));

const paymentPayload = await client.createPaymentPayload(paymentRequired);
const response = await fetch(TARGET_URL, {
  method: "POST",
  headers: {
    "content-type": "application/json",
    ...httpClient.encodePaymentSignatureHeader(paymentPayload),
  },
  body: bodyText,
});
const body = await response.json();
if (!response.ok || body?.paid !== true) {
  throw new Error(`Payment failed HTTP ${response.status}: ${JSON.stringify(body)}`);
}

const evidence = {
  verifiedAt: new Date().toISOString(),
  payer: account.address,
  recipient: payTo,
  settlement: httpClient.getPaymentSettleResponse((name) => response.headers.get(name)),
  response: body,
};
mkdirSync(dirname(EVIDENCE_FILE), { recursive: true });
writeFileSync(EVIDENCE_FILE, `${JSON.stringify(evidence, null, 2)}\n`, "utf8");
console.log(JSON.stringify(evidence, null, 2));
