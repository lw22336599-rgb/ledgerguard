...
<body>
  <main>
    ...
    <section class="hero">
      ...
      <div class="links"><a href="/openapi.json">OpenAPI</a><a href="/v1/networks">Network registry</a><a href="/ready">Live readiness</a></div>
    </section>
    <section class="panel">
      <div>
        <p class="step">LIVE DEMO</p>
        <h2>Check a USDC payment intent</h2>
        <p class="muted">This creates unsigned calldata and runs the same policy engine available through the API.</p>
      </div>
      <form id="preflight">
        ...
        <label>Amount (USDC)<input id="amount" value="1.00" required inputmode="decimal"></label>
        <label>Policy limit (USDC)<input id="limit" value="10.00" required inputmode="decimal"></label>
    ...
    <footer>Testnet software · Mainnet disabled pending official Arc parameters and an explicit release review.</footer>
  </main>
...
export const demoJs = `const form=document.querySelector("#preflight");const result=document.querySelector("#result");const usdc="0x3600000000000000000000000000000000000000";const units=(v)=>{if(!/^\\d+(\\.\\d{0,6})?$/.test(v))throw new Error("Use a positive number with up to 6 decimals.");const [w,f=""]=v.split(".");return(BigInt(w)*1000000n+BigInt((f+"000000").slice(0,6))).toString()};const pad=(v)=>v.replace(/^0x/,"").toLowerCase().padStart(64,"0");form.addEventListener("submit",async(e)=>{e.preventDefault();const button=form.querySelector("button");button.disabled=true;result.textContent="Checking Arc Testnet policy…";try{const recipient=document.querySelector("#recipient").value;const amount=units(document.querySelector("#amount").value);const limit=units(document.querySelector("#limit").value);const data="0xa9059cbb"+pad(recipient)+BigInt(amount).toString(16).padStart(64,"0");const response=await fetch("/v1/preflight",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({network:"arcTestnet",to:usdc,data,valueWei:"0",intent:{action:"transfer",expectedRecipient:recipient,expectedAssetAddress:usdc,expectedAmountMicroUsdc:amount,purpose:"LedgerGuard browser demo"},policy:{requireSimulation:false,maxAmountMicroUsdc:limit}})});const body=await response.json();result.textContent=JSON.stringify(body,null,2);result.style.borderColor=body.decision==="ALLOW"?"#82f4bd":body.decision==="BLOCK"?"#ff6b6b":"#ffb86b"}catch(error){result.textContent="Error: "+error.message;result.style.borderColor="#ff6b6b"}finally{button.disabled=false}});`;
