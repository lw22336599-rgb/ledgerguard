...
function sha3KeccakHex(input: string): string {
  // EIP-55 hashes the ASCII lowercase hexadecimal text, not the decoded
  // 20-byte address. Hashing `0x${input}` as hex bytes rejects valid mixed-case
  // checksums generated by standard EVM libraries.
  return keccak256(stringToBytes(input)).slice(2);
}
